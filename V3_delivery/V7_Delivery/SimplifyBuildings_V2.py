...
# Modified:    05/16/2016 Roslyn Dunn
#              Checked for existence of files to enable a re-start
# ---------------------------------------------------------------------------
...
def main(input_fc, output_fgdb, output_fc_name, delete_intermediate_results):
    try:
        ...
        if not arcpy.Exists(buff1_output):
        ...
        if not arcpy.Exists(single_part_features):
        ...
        if not arcpy.Exists(buff2_output):
        ...
        if not arcpy.Exists(large_buildings):
        ...
        if not arcpy.Exists(small_buildings):
        ...
        if not arcpy.Exists(simplify_once):
