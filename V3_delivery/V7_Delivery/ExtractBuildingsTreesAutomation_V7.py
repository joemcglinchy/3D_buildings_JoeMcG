...
# Modified:    if program was launched from desktop it uses the same Make Layer/Select Layer workflow. if it is Pro, I converted that workflow into a SetNull operation
#              since the desktop workflow seems to cause Pro to crash and simply does not work. Also, came across another bug when using LAS as input.. 
#              the regularize option, if turned on, would make it crash in the regularize step because it was looking for the DSM mosaic which didn't exist. 
#              I reformatted the check for the cellsize to look for las_md if the input was LAS, and also to see if all_buildings_final exists. If it doesn't, it continues.
# ---------------------------------------------------------------------------
...
def extract_buildings_trees(pp_params):
    ...
    try:
        ...
        if "ARCGISPRO" in sys.executable.upper():
            mask_null = os.path.join(out_subfolder_path, "masknull" + oid + ".tif")
            out_raster = arcpy.sa.SetNull(isnull_newma1, isnull_newma1, "Value = 0"); out_raster.save(mask_null)
            #out_raster = arcpy.sa.SetNull("IsNullnewm1.tif", "IsNullnewm1.tif", "Value = 0"); out_raster.save(r"J:\Testing\utah_lidar\test_pro1_4_3\1\setnull2.tif")
        else:
        ...
        if "TREES" in featureextraction.upper():
            ...
            if "ARCGISPRO" in sys.executable.upper():
                treenull = os.path.join(out_subfolder_path, "treenull" + oid + ".tif")
                out_raster = arcpy.sa.SetNull(isnull_setnu1, isnull_setnu1, "Value = 0"); out_raster.save(treenull)
                #out_raster = arcpy.sa.SetNull("IsNullnewm1.tif", "IsNullnewm1.tif", "Value = 0"); out_raster.save(r"J:\Testing\utah_lidar\test_pro1_4_3\1\setnull2.tif")
            else:
                treenull = "treeNULL" + oid
                #arcpy.MakeRasterLayer_management(isnull_newma1, mask_null, "", "", "")
                arcpy.MakeRasterLayer_management(isnull_newma1, treenull)
        ...
        if not "ARCGISPRO" in sys.executable.upper():
            ...
            tbinfo = traceback.format_tb(tb)[0]
            arcpy.AddError(
                "PYTHON ERRORS:\nTraceback Info:\n {0}\nError Info:\n {1}: {2}".format(tbinfo, sys.exc_info()[0], sys.exc_info()[1]))
            messages = arcpy.GetMessages()
            arcpy.AddError("GP Messages:\n {0}".format(messages))
            raise
    ...
    try:
        ...
        if second_parameter == "ALL RETURNS" or second_parameter == "LAST RETURNS":
            ...
            if not arcpy.Exists(las_raster_type_file):
                ...
                desc_lasd = arcpy.Describe(lasd)
                spat_ref_lasd = desc_lasd.SpatialReference
                set_pixel_size(las_raster_type_file, las_point_spacing, spat_ref_lasd.linearUnitName.upper())
                del desc_lasd, spat_ref_lasd
            else:
        ...
        if regularize_buildings == "true":
            ...
            # Don't bother if both feature classes already exist or if all_buildings_final does NOT exist
            if not arcpy.Exists(buildings_to_reg) or not arcpy.Exists(buildings_reg):                
                ...
                if second_parameter == "ALL RETURNS" or second_parameter == "LAST RETURNS":
                    # Input is LAS
                    cellsize_result = arcpy.GetRasterProperties_management(las_md, property_type="CELLSIZEX", band_index="")
                else:
                ...
                # if all_buildings_final exists, do the regularization
                if arcpy.Exists(all_buildings_final):
                    regularize_some_buildings(all_buildings_final, buildings_to_reg, buildings_reg, expression, cellsize, out_folder_path)
                else:
                    arcpy.AddMessage("\n no buildings detected so skipping regularization step\n")
            else:
