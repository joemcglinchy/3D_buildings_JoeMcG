...
#              Ensured correct execution from Desktop 10.4 and Pro 1.2 environments by including BuildRasterAttributeTable around line 340
...
#              Added try-except around arcpy.AddGeometryAttributes to add fields as GEODESIC. This may result in other errors if neither condition is appropriate
    ...
    try:
        ...
        if not arcpy.Exists(clip_dsm_raster):
            if las_input:
                ...
                arcpy.MakeLasDatasetLayer_management(lasd, out_layer="DSM_LASD_Layer",
                                                     class_code="0;1;2;3;4;5;6",
                                                     return_values=return_val, no_flag="true",
                                                     synthetic="true", keypoint="true",
                                                     withheld="false", surface_constraints="")
        ...
        if not arcpy.Exists(clip_dtm_raster):
            if las_input:
                ...
                arcpy.MakeLasDatasetLayer_management(lasd, out_layer="DTM_LASD_Layer", class_code="2;8",
                                                     return_values="", no_flag="true", synthetic="true",
                                                     keypoint="true", withheld="false", surface_constraints="")
                # messages = arcpy.GetMessages()
        ...
        # setnull_divi1 = os.path.join(out_subfolder_path, "SetNull_Divi1" + oid)
        setnull_divi1 = SetNull(divide_minus1, flt_raster_one, "VALUE<2")

        arcpy.Delete_management(flt_raster_one)

        # check to see if any heights are > 2 meters, and if not, then we're done
        if setnull_divi1.maximum <= 0:
        ...
        isnull_newma1 = os.path.join(out_subfolder_path, "IsNullnewm" + oid)
        ...
        disbd = os.path.join(out_subfolder_path, "disBD" + oid)
        ...
        disbdnull = os.path.join(out_subfolder_path, "disBDnull" + oid)
        ...
        rdsm = os.path.join(out_subfolder_path, "Negatediff" + oid)
        arcpy.gp.Negate_sa(disbdnull, rdsm)

        # Flow Direction
        flowdir_filt1 = os.path.join(out_subfolder_path, "FlowDirFlt" + oid)
        ...
        isocluster2 = os.path.join(out_subfolder_path, "isocluster" + oid)
        ...
        if buildingsfound:
            # Set Null
            setnullbd = os.path.join(out_subfolder_path, "SetNullbd" + oid)
            ...
            try:
                ...
                arcpy.AddGeometryAttributes_management(bdiso, "AREA_GEODESIC;PERIMETER_LENGTH_GEODESIC", "METERS", "SQUARE_METERS", "")
        ...
        if "TREES" in featureextraction.upper():
            isnull_setnu1 = os.path.join(out_subfolder_path, "IsNullSetN" + oid)
            if buildingsfound:
                # Is Null
                isnull_isocl1 = os.path.join(out_subfolder_path, "IsNullisoc" + oid)
                ...
                con_isnull_i1 = os.path.join(out_subfolder_path, "ConIsNulli" + oid)
            ...
            # Euclidean Distance
            eucdist_make1 = os.path.join(out_subfolder_path, "EucDistMak" + oid)
            arcpy.gp.EucDistance_sa(treenull, eucdist_make1, "", treenull, "")
            arcpy.Delete_management(treenull)

            # Set Null
            setnull_eucd1 = os.path.join(out_subfolder_path, "SetNulEucD" + oid)
            if "FOOT" in linear_unit:
            ...
            # Focal Statistics
            focalst_setn1 = os.path.join(out_subfolder_path, "FocalStStN" + oid)
            arcpy.gp.FocalStatistics_sa(setnull_eucd1, focalst_setn1, "Circle 3 CELL", "MAXIMUM", "DATA")

            # Minus
            minus_focals1 = os.path.join(out_subfolder_path, "MinusFoclS" + oid)
            ...
            # Equal To
            equalto_minu1 = os.path.join(out_subfolder_path, "EqualToMin" + oid)
            arcpy.gp.EqualTo_sa(minus_focals1, "0", equalto_minu1)

            # Set Null
            setnull_equa1 = os.path.join(out_subfolder_path, "SetNulEqua" + oid)
            ...
            # Int
            int_setnull_1 = os.path.join(out_subfolder_path, "IntSetNull" + oid)
...
def set_pixel_size(raster_type_file, las_point_spacing):
    ...
    # generated properly.
    try:
        # Get a good number for pixel size of the MD. It should be a minimum of 3.
...
def get_las_point_spacing(las_path, lasd, output_path, output_gdb_name):
...
def get_las_boundary(las_path, lasd, md_name, las_raster_type, file_gdb, surface_constraint_fc):
    try:
        ...
        #    Update the Boundary
        arcpy.BuildFootprints_management(md_name,where_clause="#", reset_footprint="RADIOMETRY", min_data_value="-100",
...
def main(first_parameter, second_parameter, z_unit, featureextraction, out_folder_path, processing_unit_length,
    ...
    try:
        ...
        if second_parameter == "ALL RETURNS" or second_parameter == "LAST RETURNS":
            ...
            arcpy.AddMessage("\n las_point_spacing: {0}".format(str(las_point_spacing)))
            ...
            if not arcpy.Exists(las_raster_type_file):
                ...
                set_pixel_size(las_raster_type_file, las_point_spacing)
            else:
        ...
        if "ARCMAP" in executed_from or "ARCCATALOG" in executed_from or \
