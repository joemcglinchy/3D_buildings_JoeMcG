...
def createlaslayers(inLASD):
    ...
    arcpy.MakeLasDatasetLayer_management(inLASD, "LRDSMLASD", "1;2;4", "Last Return", "", "", "", "", "", "")
    if arcpy.Exists("DTMLASD"):
        if arcpy.Exists("LRDSMLASD"):
...
with arcpy.da.SearchCursor(buildingFootprints, fields) as sc:
    for i, row in enumerate(sc):
        try:
            ...
            tp = os.path.join(outputWS, "tp_{0}".format(i))
            ...
            out_points = "in_memory/clipPoints_{0}".format(i)
            ...
            out_surf_tin = os.path.join(arcpy.env.scratchFolder, "surfTin_{0}".format(i))
            ...
            minMaxElevTable = "in_memory/minMaxElev_{0}".format(i)
            ...
            out_poly3d = "in_memory/out_poly3d_{0}".format(i)
            arcpy.FeatureTo3DByAttribute_3d(tp, out_poly3d, "MIN", "")

            # TODO INT determine whether or not ground TIN can be removed from process and multipatch be produced
            # make ground TIN
            gnd_feats_tin = "{} Shape.Z Hard_Clip <None>;".format(out_poly3d)
            out_gnd_tin = os.path.join(arcpy.env.scratchFolder, "gndTin_{0}".format(i))
...
#TODO Geof7015: delete all buildingMP* files that exist in the output workspace
